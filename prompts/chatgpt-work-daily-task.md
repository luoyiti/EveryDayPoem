...
- 选择中国古代真实存在、可可靠校勘的诗、词、曲或短篇文言文；优先意境鲜明、篇幅适中、适合普通成年读者每日学习的冷门佳作。
- 正文生成前至少准备 3 个候选。每个候选都运行：

  `npm run check:exclusion -- --title "篇名" --author "作者" --incipit "首句"`

- 只有退出码 `0`，且不在学习记录和现有诗库中的候选可继续。退出码 `2` 立即弃用，不得通过改题、节选、同词牌或故事化复述绕过。
- 最终候选再人工核对标题、作者、朝代、首句、别名、词牌和常见异文；有疑义就换篇。
...
1. 先写短视觉 brief：时间、地点、天气、色温、材质、核心意象、视觉命题与诗中转折；不得复制已有篇目的构图。
...
4. 更新 `src/data/daily.js` 与 `data/learning-record.json`，当天新篇必须是学习记录第一项，日期使用 `Asia/Shanghai`。
...
仅报告：今日篇名与作者、三个候选及排除校验结果、引用来源、改动文件、`verify:offline` 结果、本地依赖/build 状态、远端构建/部署结果、任务基线 SHA、唯一 GitHub 提交 SHA、Vercel 生产 URL。任何未执行的检查都必须明确写“未执行”及原因，不得用“应该可以”代替证据。
