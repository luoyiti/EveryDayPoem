...
import "./hepu-starry-sea.css";

const STORAGE_KEY = "daily-poetry-progress-v1";
const normalize = (value = "") => value.replace(/[，。！？；：、“”‘’（）()《》\s]/g, "");
const marks = ["陆", "海", "息", "书", "岸"];
const captions = ["大雨 · 坏桥", "无月 · 星河", "稚子 · 鼾睡", "别本 · 必济", "七月 · 合浦"];

export function HepuStarrySeaPage({ poem, onNavigate }) {
  ...
  return (
    <main className="hs-page">
      <img className="hs-bg" src={poem.image} alt="" aria-hidden="true" />
      <div className="hs-night" aria-hidden="true" />
      <div className="hs-waterline" aria-hidden="true" />

      <header className="hs-header">
        <a className="hs-brand" href="#hepu-starry-sea" aria-label="每日古诗文首页">
          <strong>每日古诗文</strong><span>一夜海宿 · 星河为界</span>
        </a>
        <div className="hs-meta"><span>2026.09.16 · {poem.dynasty} · {poem.genre}</span><button onClick={() => setHistory(true)}><BookOpenText size={17} /> 往日</button></div>
      </header>

      <section className="hs-stage" aria-labelledby="hs-title">
        <div className="hs-title-block">
          <small>{poem.form}</small>
          <h1 id="hs-title">{poem.title}</h1>
          <p>{poem.author}</p>
          <div className="hs-coordinate"><span>海康</span><i /><span>合浦</span></div>
        </div>

        <div className="hs-reading" aria-live="polite">
          <div className="hs-reading-head"><span>航记 {String(active + 1).padStart(2, "0")}</span><small>{captions[active]}</small></div>
          <p>{poem.lines[active]}</p>
          <button onClick={() => setActive(Math.min(active + 1, poem.lines.length - 1))} disabled={active === poem.lines.length - 1}>
            {active === poem.lines.length - 1 ? "已抵合浦" : "沿记述继续 →"}
          </button>
        </div>

        <aside className="hs-note">
          <span>字词与章法</span>
          <h2>{poem.notes[active].term}</h2>
          <p>{poem.notes[active].text}</p>
        </aside>

        <nav className="hs-route" aria-label="五段行程">
          {marks.map((mark, index) => (
            <button key={mark} className={active === index ? "is-active" : ""} onClick={() => setActive(index)} aria-current={active === index ? "step" : undefined}>
              <b>{mark}</b><span>{captions[index]}</span><i>{String(index + 1).padStart(2, "0")}</i>
            </button>
          ))}
        </nav>
      </section>

      <nav className="hs-tools" aria-label="作品学习工具">
      ...
      {detail && <aside className="hs-detail" role="dialog" aria-modal="true" aria-label={detail === "translation" ? "译文" : "赏析"}>
        <button className="hs-scrim" onClick={() => setDetail(null)} aria-label="关闭" />
      ...
      {study && <div className="hs-study" role="dialog" aria-modal="true" aria-label={`${poem.title}${study === "dictation" ? "默写" : "背诵"}`}>
        <button className="hs-scrim" onClick={() => setStudy(null)} aria-label="关闭学习层" />
        <section className="hs-study-sheet">
          <header><button onClick={() => setStudy(null)}><ArrowLeft size={17} /> 回到海夜</button><span>{study === "dictation" ? "默写" : "背诵"}</span></header>
          {study === "recitation" ? <div className="hs-recite"><small>{marks[reciteIndex]} · {reciteIndex + 1} / {poem.lines.length}</small><button className={hidden ? "is-hidden" : ""} onClick={() => setHidden((value) => !value)}>{poem.lines[reciteIndex]}</button><p>{hidden ? poem.studyCopy.recitationHint : poem.notes[reciteIndex].text}</p><div><button onClick={() => setHidden((value) => !value)}>{hidden ? "显出原文" : "覆去原文"}</button><button onClick={nextRecitation}>{reciteIndex === poem.lines.length - 1 ? "完成背诵" : "进入下一段"}</button></div></div>
          : <div className="hs-dictation"><h2>{poem.studyCopy.dictationTitle}</h2>{poem.lines.map((line, index) => { const correct = normalize(answers[index]) === normalize(line); return <label key={index}><span>{marks[index]} · {captions[index]}</span><textarea rows={4} value={answers[index]} onChange={(event) => { const next = [...answers]; next[index] = event.target.value; setAnswers(next); setChecked(false); }} placeholder="默写这一段" />{checked && (correct ? <Check size={18} /> : <small>{line}</small>)}</label>; })}{checked && <p>{score === poem.lines.length ? poem.studyCopy.dictationSuccess : `写对 ${score} 段，继续核对。`}</p>}<button className="hs-submit" onClick={checkDictation}>核对默写</button></div>}
        </section>
      </div>}

      <aside className={`hs-history ${history ? "is-open" : ""}`} aria-hidden={!history} inert={history ? undefined : true}>
