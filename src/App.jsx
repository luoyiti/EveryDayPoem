...
function Brand({ dark = true }) {
  return (
    <div className={`brand ${dark ? "brand--light" : "brand--dark"}`}>
      <span>每日古诗文</span>
      <i aria-hidden="true" />
    </div>
  );
    ...
    <aside className={`history-drawer ${open ? "is-open" : ""}`} aria-hidden={!open} inert={open ? undefined : true}>
      ...
      <div className="history-list">
        {poems.map((poem, index) => (
          <button
            className={`history-row ${poem.id === currentId ? "is-current" : ""}`}
            key={poem.id}
          ...
          >
            ...
            <span className="history-copy">
              ...
              <em>{poem.learnedAt}</em>
            </span>
            <span className="history-status" aria-label={progress[poem.id]?.completed ? "已完成" : "未完成"}>
              {progress[poem.id]?.completed ? <Check size={15} /> : "阅"}
            </span>
          </button>
        ))}
      </div>
...
function StudyOverlay({ poem, mode, onClose, onComplete }) {
  ...
  function submitDictation() {
    setChecked(true);
    if (score === poem.lines.length) onComplete("dictation");
  }

  function advanceRecitation() {
    if (reciteIndex === poem.lines.length - 1) {
      onComplete("recitation");
      onClose();
      return;
    }
          ...
          <div className="dictation-content">
            ...
            <div className="dictation-lines">
              {poem.lines.map((line, index) => {
                const isCorrect = answers[index].replace(/[，。！？、\s]/g, "") === line.replace(/[，。！？、\s]/g, "");
                return (
                  <label key={line} className={checked ? (isCorrect ? "is-correct" : "is-wrong") : ""}>
                    <span>{index + 1}</span>
                    <input
                      value={answers[index]}
                      onChange={(event) => {
                      ...
                      autoFocus={index === 0}
                    />
                    {checked && !isCorrect && <small>{line}</small>}
                    {checked && isCorrect && <Check size={18} />}
                  </label>
                );
              })}
            </div>
...
function DetailPanel({ poem, mode, onClose }) {
  ...
  return (
    <section className="detail-panel" aria-live="polite">
      <header><span>{title}</span><button onClick={onClose} aria-label={`关闭${title}`}><X size={17} /></button></header>
      <p>{content}</p>
    </section>
  );
}
...
function MapleNight({ poem, onHistory, onStudy }) {
  ...
  function ringBell() {
    playBell();
    setBellActive(true);
    window.setTimeout(() => setBellActive(false), 2400);
  }

  return (
    <main className="poem-page maple-night">
      <PageImage src={poem.image} position="center center" />
      <div className="maple-tint" aria-hidden="true" />
      <Brand />

      <section className="maple-reading" aria-labelledby="poem-title">
        <header className="poem-heading">
          <span>{poem.dynasty}</span>
          <h1 id="poem-title">{poem.title}</h1>
          <p>{poem.dynasty} · {poem.author}</p>
        </header>

        <div className="maple-verses">
          {poem.lines.map((line, index) => (
            <div className={`maple-verse ${activeLine === index ? "is-active" : ""}`} key={line}>
              {activeLine === index && detail === "annotation" && (
                <p className="verse-note"><strong>{poem.notes[index].term}</strong>{poem.notes[index].text}</p>
              )}
              <button onClick={() => { setActiveLine(index); setDetail("annotation"); }}>
                <span>{line}</span>
              </button>
              {index === poem.lines.length - 1 && (
                <button className="dictation-cta" onClick={() => onStudy("dictation")}>开始默写</button>
              )}
            </div>
          ))}
        </div>
      ...
      <button className={`bell-button ${bellActive ? "is-ringing" : ""}`} onClick={ringBell}>
        <BellRinging size={18} weight="light" />
        <span>{bellActive ? "钟声正越过江面" : "听一声夜半钟"}</span>
      </button>

      <DetailPanel poem={poem} mode={detail} onClose={() => setDetail("annotation")} />

      <nav className="maple-rail" aria-label="诗词学习">
        <button className="history-trigger" onClick={onHistory}>
          <BookOpenText size={20} weight="light" />
          <span>诗笺集</span>
        </button>
        <div className="rail-items">
          <button className={detail === "annotation" ? "is-active" : ""} onClick={() => setDetail("annotation")}><span>注释</span><i /></button>
          <button className={detail === "translation" ? "is-active" : ""} onClick={() => setDetail("translation")}><span>译文</span><i /></button>
          <button className={detail === "appreciation" ? "is-active" : ""} onClick={() => setDetail("appreciation")}><span>赏析</span><i /></button>
          <button onClick={() => onStudy("recitation")}><span>背诵</span><i /></button>
        </div>
      </nav>
    </main>
  );
...
function SnowRiver({ poem, onHistory, onStudy }) {
  ...
  return (
    <main className="poem-page snow-river">
      <PageImage src={poem.image} position="center center" />
      <Brand dark={false} />
      <button className="snow-history" onClick={onHistory}><List size={18} /> 往日诗笺</button>
      <div className="snow-index">贰<br /><span>孤舟 · 寒江</span></div>

      <section className="snow-composition" aria-labelledby="snow-title">
        <header>
          <span>{poem.dynasty} · {poem.author}</span>
          <h1 id="snow-title">{poem.title}</h1>
        </header>
        <div className="snow-verses">
          {poem.lines.map((line) => <p key={line}>{line}</p>)}
        </div>
      </section>

      <aside className={`snow-meaning ${showMeaning ? "is-open" : ""}`}>
        <span>诗意</span>
        <p>{poem.translation}</p>
      </aside>
      <div className="snow-actions">
        <button onClick={() => setShowMeaning((value) => !value)}>{showMeaning ? "收起诗意" : "展开诗意"}</button>
        <button onClick={() => onStudy("recitation")}><Feather size={17} /> 开始背诵</button>
      </div>
    </main>
  );
}
...
function SpringDawn({ poem, onHistory, onStudy }) {
  ...
  return (
    <main className="poem-page spring-dawn">
      <PageImage src={poem.image} position="center center" />
      <div className="spring-wash" aria-hidden="true" />
      <Brand dark={false} />
      <button className="spring-history" onClick={onHistory}><BookOpenText size={18} /> 诗笺集</button>
      <section className="spring-copy" aria-labelledby="spring-title">
        <header>
          <span>晨起 · 第三笺</span>
          <h1 id="spring-title">{poem.title}</h1>
          <p>{poem.dynasty} · {poem.author}</p>
        </header>
        <div className="spring-verses">
          {poem.lines.map((line, index) => (
            <button className={activeLine === index ? "is-active" : ""} key={line} onClick={() => setActiveLine(index)}>{line}</button>
          ))}
        </div>
        <p className="spring-note"><strong>{poem.notes[activeLine].term}</strong>{poem.notes[activeLine].text}</p>
        <div className="spring-actions">
          <button onClick={() => onStudy("recitation")}><SpeakerHigh size={18} /> 轻声诵读</button>
          <button onClick={() => onStudy("dictation")}>开始默写</button>
        </div>
      </section>
    </main>
  );
...
export function App() {
  ...
  useEffect(() => {
    const onHashChange = () => {
      ...
      if (poemsById[next]) setCurrentId(next);
    };
  ...
  return (
    <div className="app-shell">
      ...
      <HistoryDrawer open={historyOpen} onClose={() => setHistoryOpen(false)} onSelect={selectPoem} currentId={poem.id} progress={progress} />
      <div className={`drawer-scrim ${historyOpen ? "is-visible" : ""}`} onClick={() => setHistoryOpen(false)} />
      <StudyOverlay poem={poem} mode={studyMode} onClose={() => setStudyMode(null)} onComplete={completeStudy} />
    </div>
  );
}
