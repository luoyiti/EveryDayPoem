...
function LanxiMoon({ poem, onHistory, onStudy }) {
  ...
  return (
    <main className="poem-page lanxi-moon">
      <PageImage src={poem.image} position="center center" />
      <div className="lanxi-shade" aria-hidden="true" />
      <Brand />
      <button className="lanxi-history" onClick={onHistory}><BookOpenText size={18} weight="light" /> 往日诗笺</button>
      <header className="lanxi-heading"><span>兰溪 · 春夜</span><h1>{poem.title}</h1><p>{poem.dynasty} · {poem.author}</p></header>
      <section className="lanxi-reading" aria-label="兰溪棹歌逐句阅读">
        <div className="lanxi-verses">
          {poem.lines.map((line, index) => (
            <button key={line} className={activeLine === index ? "is-active" : ""} onClick={() => { setActiveLine(index); setDetail("annotation"); }} aria-current={activeLine === index ? "step" : undefined}>
              <span className="lanxi-number">{String(index + 1).padStart(2, "0")}</span><span className="lanxi-line">{line}</span><i aria-hidden="true" />
            </button>
          ))}
        </div>
        <div className="lanxi-annotation" aria-live="polite"><strong>{poem.notes[activeLine].term}</strong><p>{poem.notes[activeLine].text}</p></div>
      </section>
      {detail !== "annotation" && <aside className="lanxi-detail" aria-live="polite"><header><span>{detailTitle}</span><button onClick={() => setDetail("annotation")} aria-label={`关闭${detailTitle}`}><X size={17} /></button></header><p>{detailContent}</p></aside>}
      <nav className="lanxi-tools" aria-label="诗词学习"><button className={detail === "annotation" ? "is-active" : ""} onClick={() => setDetail("annotation")}>注释</button><button className={detail === "translation" ? "is-active" : ""} onClick={() => setDetail("translation")}>译文</button><button className={detail === "appreciation" ? "is-active" : ""} onClick={() => setDetail("appreciation")}>赏析</button><button onClick={() => onStudy("recitation")}>背诵</button><button onClick={() => onStudy("dictation")}>默写</button></nav>
    </main>
  );
}
